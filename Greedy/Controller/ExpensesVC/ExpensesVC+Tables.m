...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    if (tableView == self.categoriesTable) {
        ...
        cell.iconView.backgroundColor = [[ColorsManager getAllColors] objectAtIndex:category.colorIndex];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (tableView == self.categoriesTable) {
        if (![[CategoriesManager getAllCategories] count])
            return [[NSBundle.mainBundle loadNibNamed:@"NoCategoriesLabelXIB" owner:self options:nil] objectAtIndex:0];
        return nil;
    }
    ...
    if ([date isEqualToString:[NSDateFormatter localizedStringFromDate:[NSDate date] dateStyle:NSDateFormatterMediumStyle timeStyle:NSDateFormatterNoStyle]])
         date = @"Today";
    if ([date isEqualToString:[NSDateFormatter localizedStringFromDate:[NSDate.date dateByAddingTimeInterval:-86400.0] dateStyle:NSDateFormatterMediumStyle timeStyle:NSDateFormatterNoStyle]])
        date = @"Yesterday";
    UIView* headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.expensesByDatesTable.frame.size.width, 30)];
    ...
    [headerText addAttribute:NSForegroundColorAttributeName value:UIColor.darkGrayColor range:[date rangeOfString:date]];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    UIView* line = [[UIView alloc] initWithFrame:CGRectMake(50, 5, self.view.frame.size.width - 100, 1)];
    [line setBackgroundColor:UIColor.lightGrayColor];
    [footerView addSubview:line];
    UILabel* footerLabel = [[UILabel alloc] initWithFrame:CGRectMake(25, 6, self.view.frame.size.width - 50, 30)];
    NSArray <NSDate*>* dates = [ExpensesManager getExpensesDates];
    NSDate* date = [dates objectAtIndex:section];
    float total = [ExpensesManager getTotalCostBy:date];
    NSString* totalString = [[[[NSNumber numberWithFloat:total] stringValue] stringByAppendingString:@" UAH"] stringByReplacingOccurrencesOfString:@"." withString:@","];
    NSMutableAttributedString* footerText = [[NSMutableAttributedString alloc] initWithString:totalString];
    UIFont* avenir = [UIFont fontWithName:@"Avenir" size:15];
    if (self.view.frame.size.width < 375)
        avenir = [UIFont fontWithName:@"Avenir" size:14];
    [footerText addAttribute:NSFontAttributeName value:avenir range:[totalString rangeOfString:totalString]];
    [footerText addAttribute:NSForegroundColorAttributeName value:UIColor.darkGrayColor range:[totalString rangeOfString:totalString]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.destinationViewController isKindOfClass:ExpenseDetailsVC.class]) {
        ...
        if (self.expensesByDatesTable &&
            !self.expensesByDatesTable.isHidden) {
...
- (void)showCategories {
    ...
    if (![[CategoriesManager getAllCategories] count]) {
        self.noCategoriesLabel = [[NSBundle.mainBundle loadNibNamed:@"NoCategoriesLabelXIB" owner:self options:nil] objectAtIndex:0];
...
- (void)showExpensesByDates {
    ...
    if (![[ExpensesManager getExpensesDates] count]) {
        self.noExpensesLabel = [[NSBundle.mainBundle loadNibNamed:@"NoExpensesLabelXIB" owner:self options:nil] objectAtIndex:0];
...
- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[CategoriesManager getAllCategories] count]) {
        UIView* refreshView = [[NSBundle.mainBundle loadNibNamed:@"AddNewCategoryXIB" owner:self options:nil] objectAtIndex:0];
        refreshView.frame = self.refreshControl.bounds;
        [self.refreshControl addSubview:refreshView];
    }
    [self.refreshControl addTarget:self action:@selector(createNewCategory) forControlEvents:UIControlEventValueChanged];
...
- (void)tableView:(UITableView *)tableView didEndDisplayingHeaderView:(UIView *)view forSection:(NSInteger)section {
    ...
    [self.categoriesTable addSubview:self.refreshControl];
//    if (![[CategoriesManager getAllCategories] count]) {
//        [self.expenseNameTextField setEnabled:false];
//    }
//    else {
//        [self.expenseNameTextField setEnabled:true];
//    }
}
