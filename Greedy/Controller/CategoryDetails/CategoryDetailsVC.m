...
- (void)dismissSelf {
    ...
    if (!_categoryDeleted && _selectedCategory) {
        ...
        [self.headerToReload setBackgroundColor:[[ColorsManager getAllColors] objectAtIndex:_selectedColorIndex]];
...
- (void) controlLength {
    if (self.categoryName.text.length > 20) {
        [self.categoryName setText:[self.categoryName.text substringToIndex:self.categoryName.text.length - 1]];
    }
}
...
- (void) commitChanges {
    if (_selectedCategory) {
        UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"Warning"
                                                                       message:@"If you delete this category all it's expenses also will be deleted"
