...
- (void) setupFieldsAndButtons {
    ...
    [self.titleTextField addTarget:self action:@selector(controlLength) forControlEvents:UIControlEventEditingChanged];
    [self.costTextField setDelegate:self];
    [self.costTextField addTarget:self action:@selector(controlCost) forControlEvents:UIControlEventEditingChanged];
    [self.costTextField addTarget:self action:@selector(controlPoint) forControlEvents:UIControlEventEditingDidEnd];
    ...
    [self.pickCategoryButton addTarget:self action:@selector(showCategoriesCollection) forControlEvents:UIControlEventTouchUpInside];
    [self.doneWithExpenseButton addTarget:self action:@selector(commitChanges) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)paintIfNecessary {
    if (self.selectedExpense) {
        ...
        [self.dateButton setTitle:[NSDateFormatter localizedStringFromDate:self.selectedExpense.date dateStyle:NSDateFormatterMediumStyle timeStyle:NSDateFormatterNoStyle] forState:UIControlStateNormal];
        ...
        [self.doneWithExpenseButton setImage:[UIImage imageNamed:@"rubbish-bin"] forState:UIControlStateNormal];
...
- (void) controlLength {
    if (self.titleTextField.text.length > 20) {
        [self.titleTextField setText:[self.titleTextField.text substringToIndex:self.titleTextField.text.length - 1]];
    }
}
