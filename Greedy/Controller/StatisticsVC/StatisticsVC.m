...
- (void)viewDidLoad {
    ...
    [_showTotal addTarget:self action:@selector(switchToTotal) forControlEvents:UIControlEventTouchUpInside];
    [_showMonth addTarget:self action:@selector(switchToMonth) forControlEvents:UIControlEventTouchUpInside];
    [_showToday addTarget:self action:@selector(switchToToday) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)switchToMonth {
    ...
    [_categoriesTable reloadSections:[[NSIndexSet alloc] initWithIndex:0] withRowAnimation:UITableViewRowAnimationBottom];
}
...
- (NSMutableAttributedString*)getTotalString:(nullable Category*)cat {
    ...
    NSString* totalString = [[[NSNumber numberWithFloat:categoryTotal] stringValue] stringByAppendingString:@" UAH\n"];
    ...
    NSMutableAttributedString* stringPercentsAttr = [[NSMutableAttributedString alloc] initWithString:[totalString stringByAppendingString:stringPercents]];
    [stringPercentsAttr addAttribute:NSForegroundColorAttributeName value:[[ColorsManager getAllColors] objectAtIndex:category.colorIndex] range:[[totalString stringByAppendingString:stringPercents] rangeOfString:[totalString stringByAppendingString:stringPercents]]];
    [stringPercentsAttr addAttribute:NSForegroundColorAttributeName value:UIColor.blackColor range:[totalString rangeOfString:totalString]];
    UIFont* avenir = [UIFont fontWithName:@"AvenirNext-Medium" size:14];
    [stringPercentsAttr addAttribute:NSFontAttributeName value:avenir range:[[totalString stringByAppendingString:stringPercents] rangeOfString:[totalString stringByAppendingString:stringPercents]]];
...
- (void)makeActiveButtonOf:(DateType)type {
    ...
    UIColor* textColor = _selectedDateOption == Total ? _showMonth.titleLabel.textColor : _showTotal.titleLabel.textColor;
