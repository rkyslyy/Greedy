...
+ (NSArray<Expense *> *)getAllExpensesOfCategory:(NSString *)category {
  ...
  allExpenses = [allExpenses filteredArrayUsingPredicate:predicate];
  return allExpenses;
}
...
+ (NSArray<Expense *> *)getAllExpensesBy:(NSDate *)date {
  ...
  return [allExpenses filteredArrayUsingPredicate:predicate];
}
...
+ (float)getTodayCostOf:(Category *)category {
  ...
  NSArray <Expense *> *expenses = [self getAllExpensesOfCategory:category.title];
  for (Expense *expense in expenses) {
    if ([[NSDateFormatter localizedStringFromDate:expense.date
                                        dateStyle:NSDateFormatterMediumStyle
                                        timeStyle:NSDateFormatterNoStyle] isEqualToString:today]) {
      total += expense.cost;
    }
  }
  return total;
